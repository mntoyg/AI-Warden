...
| `bash` typed in PowerShell is WSL, not Git Bash | On this box `Get-Command bash` -> `C:\Windows\system32ash.exe` (WSL, kali-linux first). Everything was verified under Git Bash, so run scripts as `& "C:\Program Files\Gitinash.exe" -lc '...'` (the app's terminal panel is PowerShell 7). A codex TUI cannot be captured headlessly: under `script(1)` with no emulator answering its terminal queries it renders nothing and times out - only a real terminal rehearsal tests act 4's screens. |
